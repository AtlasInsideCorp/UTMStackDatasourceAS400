...
public class ConfigVerification {
    ...
    public static boolean isEnvironmentOk() {
        ...
        try {
            // Getting the collector key used to decrypt
            String ENCRYPTION_KEY = FileOperations.getCollectorInfo().get(Constants.COLLECTOR_KEY_HEADER)
                    + "-" + CipherUtil.AS_400_SEED_SECRET_KEY + "-" + FileOperations.getLockFile().lastModified();

            // Decrypting the servers configuration file
            String finalData = CipherUtil.encryptionByMode(FileOperations.readServersFile(), ENCRYPTION_KEY, Cipher.DECRYPT_MODE);

            // Parsing JSON Servers structure to handler class
            GenericParser gp = new GenericParser();
            serverConfigAS400 = gp.parseFrom(finalData, ServerConfigAS400.class, new ServerConfigAS400());
